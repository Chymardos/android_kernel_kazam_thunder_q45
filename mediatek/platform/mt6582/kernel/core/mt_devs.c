...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
    ...
    */
	...
	char *br_ptr;
        ...
        /*  zhoulidong add  */
		if ((br_ptr = strstr(*cmdline, "rgk_memname=")) != 0) {
			
			strcpy(rgk_mem_name,br_ptr+12);
			printk("zhou:  %s\n", br_ptr+12);
		}
	/* zhoulidong add  */
...
struct platform_device lis3de_gsensor = {
	.name	       = "lis3de_gsensor",
	.id            = -1,
};
...
//    spi_register_board_info(spi_board_devs, ARRAY_SIZE(spi_board_devs));
  ...
  if(retval != 0)
  {
	...
	retval = platform_device_register(&lis3de_gsensor);
	    printk("lis3de_gsensor device!");
	if (retval != 0)
		return retval;
#endif
