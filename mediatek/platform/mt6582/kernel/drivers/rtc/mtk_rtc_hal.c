...
//#include <mach/upmu_hw.h>
#include "../../../hardware/rtc/mt_rtc_hw.h"
...
void hal_rtc_bbpu_pwdn(void) 
{
	...
	if (!(rtc_read(RTC_PDN1) & RTC_GPIO_USER_MASK)) {
		...
		rtc_write_trigger();
                ...
                /* 1.	Enable reset-debounce (0x1000_0310 = 67D2_A357) */
                mtk_wdt_deglitch_en();
  
  	
                /* 2.	Set SRCLKENAs GPIO GPIO as Output Mode, Output Low */	
	...
	/* 4.	Switch SRCLKENAs GPIO MUX function to GPIO Mode */
