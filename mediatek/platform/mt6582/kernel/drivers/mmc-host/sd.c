...
#ifdef MTK_SDIO30_TEST_MODE_SUPPORT
#define TESTMODE_TIMEOUT    (HZ    * 60 )  /* 60s    */ 
#endif // MTK_SDIO30_TEST_MODE_SUPPORT
...
static void msdc_sd_power(struct msdc_host *host,u32 on)
{
	...
	switch(host->id){
		case 1:
			...
			//msdc_set_enio18(host,0);
		// change by anxiang.xiao for project not support hotplus bottom ele too high 2013-10-17
		#ifdef CUST_EINT_MSDC1_INS_NUM
			//msdc_ldo_power(on, MT65XX_POWER_LDO_VMCH1, VOL_3300, &g_msdc1_flash);

		#else
			msdc_ldo_power(on, MT6323_POWER_LDO_VMCH, VOL_3300, &g_msdc1_flash);  //for hot-plug by  hxh
		#endif
			break;
#if (0 == REMOVEED_FOR_MT6582)
		case 2:
...
#ifdef MTK_SDIO30_TEST_MODE_SUPPORT
static void msdc_tasklet_testmode(unsigned long arg)
{
    struct msdc_host *host = (struct msdc_host *)arg;
    int err = 0;
	int cur_temperature = 0;
    
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Enter test mode tasklet\n");
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Tasklet, host = %x\n", host);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Tasklet, host->testmode_timer = %x, host->testmode_tasklet = %x\n", &host->testmode_timer, &host->testmode_tasklet);
	...
	xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Tasklet, cur_temperature = %d, host->pre_temper = %d\n", cur_temperature, host->pre_temper);
    if(cur_temperature != -127000 && host->pre_temper != -127000 && abs(cur_temperature-host->pre_temper) > 10000) {
...
static unsigned int msdc_find_best_gear(u32 CRCResult, const u32 gear_window_size, int *gear_shift)
{
    int gear_offset, best_gear_offset = 0, cont_pass = 0, max_cont_pass = 0;
    int i;
    
    *gear_shift = 0;
    
    if(((CRCResult >> (gear_window_size - 1))&7) != 7) {
        for(i = 2*gear_window_size; i >= 0; i--) {
            if((CRCResult>>i)&1) {
                cont_pass++;
                if(cont_pass >= 3 && cont_pass > max_cont_pass) {
                    max_cont_pass = cont_pass;
                    best_gear_offset = gear_offset + max_cont_pass/2 - gear_window_size;
                }
            } else {
                cont_pass = 0;
                gear_offset = i+1;
        ...
        if(best_gear_offset == 0) {
            if(((CRCResult >> (gear_window_size*2-1))&3)>0)
                *gear_shift = -gear_window_size*2 + 2;
            else
                *gear_shift = gear_window_size*2 - 2;
            return 1;
        } else {
            *gear_shift = best_gear_offset;
        }
    }
...
static unsigned int msdc_online_tuning(struct msdc_host   *host, unsigned fn, unsigned addr)
{
    ...
    u32 eco_ver;
    u8 autocmd;
    const u32 gear_window_size = 3;
    struct mmc_command cmd = {0};
    struct mmc_data data;
    
    cmd.data = &data;
    
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Enter msdc_online_tuning \n");
    
//    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "[online_tuning] mrq = %x, cmd = %x, data = %x, id = %d\n", host->mrq, host->cmd, host->data, host->id);
    
    /* Get current delay settings */
    sdr_get_field(MSDC_PAD_TUNE, MSDC_PAD_TUNE_CMDRDLY, cmdrdly);	// Get cmd delay
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "cmd delay = %d\n", cmdrdly);
    
    rxdly0 = sdr_read32(MSDC_DAT_RDDLY0);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "DAT read delay line 0 = %x\n", rxdly0);
    if (eco_ver = sdr_read32(MSDC_ECO_VER) >= 4) {
        dat0rddly = (rxdly0 >> 24) & 0x1F;
        dat1rddly = (rxdly0 >> 16) & 0x1F;
        dat2rddly = (rxdly0 >>  8) & 0x1F;
        dat3rddly = (rxdly0 >>  0) & 0x1F;
    } else {   
        dat0rddly = (rxdly0 >>  0) & 0x1F;
        dat1rddly = (rxdly0 >>  8) & 0x1F;
        dat2rddly = (rxdly0 >> 16) & 0x1F;
        dat3rddly = (rxdly0 >> 24) & 0x1F;
    }
    ...
    autocmd = host->autocmd;
    host->autocmd = MSDC_AUTOCMD53;
    cmd.arg |= fn << 28;
    //cmd.arg |= incr_addr ? 0x04000000 : 0x00000000;	// fixed address
    cmd.arg |= addr << 9;
    cmd.arg |= 0x08000001;		/* block mode */
...
_retry:
    ...
    cmd.arg &= ~0x80000000;	// Read
    data.flags = MMC_DATA_READ;
    
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "enter command start\n");
    if (msdc_command_start(host, &cmd, 0, CMD_TIMEOUT) != 0) {
        host->autocmd = autocmd;
    ...
    /* Check cmd gear */
    retry = msdc_find_best_gear(CRCStatus_CMD, gear_window_size, &gear_shift);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "CRCStatus_CMD = %d, gear_shift = %d\n", CRCStatus_CMD, gear_shift);
    if(gear_shift) {
        /* Reset delay gear */
        cmdrdly = (cmdrdly + gear_shift) & 0x1F;
        sdr_set_field(MSDC_PAD_TUNE, MSDC_PAD_TUNE_CMDRDLY, cmdrdly);	// Set cmd delay
    }
    
    reset = 0;
    /* Check read gear */
    retry |= msdc_find_best_gear(cmd.resp[0], gear_window_size, &gear_shift);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "cmd.resp[0] = %d, gear_shift = %d\n", cmd.resp[0], gear_shift);
    if(gear_shift) {
        dat0rddly = (dat0rddly + gear_shift) & 0x1F;
        reset++;
    }
    retry |= msdc_find_best_gear(cmd.resp[1], gear_window_size, &gear_shift);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "cmd.resp[1] = %d, gear_shift = %d\n", cmd.resp[1], gear_shift);
    if(gear_shift) {
        dat1rddly = (dat1rddly + gear_shift) & 0x1F;
        reset++;
    }
    retry |= msdc_find_best_gear(cmd.resp[2], gear_window_size, &gear_shift);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "cmd.resp[2] = %d, gear_shift = %d\n", cmd.resp[2], gear_shift);
    if(gear_shift) {
        dat2rddly = (dat2rddly + gear_shift) & 0x1F;
        reset++;
    }
    retry |= msdc_find_best_gear(cmd.resp[3], gear_window_size, &gear_shift);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "cmd.resp[3] = %d, gear_shift = %d\n", cmd.resp[3], gear_shift);
    if(gear_shift) {
        dat3rddly = (dat3rddly + gear_shift) & 0x1F;
        reset++;
    }
    ...
    if(retry) {
        xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "goto _retry (Write) \n");
        goto _retry;
    }
...
static int msdc_do_request(struct mmc_host*mmc, struct mmc_request*mrq)
{
    ...
    if(host->mmc->card) {
        if(host->mmc->card->type == MMC_TYPE_SDIO) {
            spin_lock_irqsave(&host->clk_gate_lock, flags);
            if(host->testmode_timeout == 0) {
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Trigger Timer[B], expires = %d, timeout = %d\n",host->testmode_timer.expires, host->testmode_timeout);
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Timer Function = %x\n",host->testmode_timer.function);
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, host = %x, host->mrq = %x, host->id = %d\n", host, host->mrq, host->id);
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, host->mmc = %x\n", host->mmc);
//                if(host->mmc) {
//                    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, host->mmc->card = %x\n", host->mmc->card);
//                    if(host->mmc->card) {
//                        xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, host->mmc->card->type = %d\n", host->mmc->card->type);
//                    }
//                }
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, cmd->opcode = %d, cmd->arg = %x, cmd->flags = %x\n", cmd->opcode, cmd->arg, cmd->flags);
//                if(data)
//                    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, data->sg->dma_address = %x, data->sg->offset = %x, data->sg->page_link = %x\n", data->sg->dma_address, data->sg->offset, data->sg->page_link);
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Do request, host->testmode_timer = %x, host->testmode_tasklet = %x\n", &host->testmode_timer, &host->testmode_tasklet);
                mod_timer(&host->testmode_timer, jiffies + TESTMODE_TIMEOUT);
                host->testmode_timeout = 1;
//                xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Trigger Timer[E], expires = %d, timeout = %d\n",host->testmode_timer.expires, host->testmode_timeout);
            }
            spin_unlock_irqrestore(&host->clk_gate_lock, flags); 
        }
    }
...
static void msdc_enable_cd_irq(struct msdc_host *host, int enable)
{
    ...
    #ifdef CUST_EINT_MSDC1_INS_NUM
     mt_set_gpio_mode(GPIO_MSDC1_INSI, GPIO_MODE_04);
    mt_set_gpio_dir(GPIO_MSDC1_INSI, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_MSDC1_INSI, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_MSDC1_INSI, GPIO_PULL_DOWN);
	#endif
#ifdef CUST_EINT_MSDC1_INS_NUM
	...
	if(enable){
		if(host->id == 1 && (MSDC_SD == hw->host_function) && (hw->flags & MSDC_CD_PIN_EN)){
			...
			printk("SD card detection eint resigter.");
		}
...
#ifdef MTK_SDIO30_TEST_MODE_SUPPORT
/* This is called by run_timer_softirq */
static void msdc_timer_testmode(unsigned long data)
{
    struct msdc_host *host = (struct msdc_host *)data;
    unsigned long flags;
    
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Time out, start regular tuning, testmode_timeout = %d\n", host->testmode_timeout);
    xlog_printk(ANDROID_LOG_DEBUG, "SDIO_TEST_MODE", "Time out, host->testmode_timer = %x, host->testmode_tasklet = %x\n", &host->testmode_timer, &host->testmode_tasklet);

    spin_lock_irqsave(&host->clk_gate_lock, flags);
    
    host->testmode_timeout = 0;
    
    tasklet_hi_schedule(&host->testmode_tasklet);
    
    del_timer(&host->testmode_timer);
    spin_unlock_irqrestore(&host->clk_gate_lock, flags); 
}
#endif // MTK_SDIO30_TEST_MODE_SUPPORT
...
/*
	...
	if((host->hw->host_function == MSDC_SD) && (host->hw->flags & MSDC_CD_PIN_EN)){
		...
		// change by anxiang.xiao for project not support hotplus bottom ele too high 2013-10-17
		#ifdef CUST_EINT_MSDC1_INS_NUM
			//msdc_sd_power(host,0);  //for hot-plug project by hxh
...
#ifdef MTK_SDIO30_TEST_MODE_SUPPORT
    host->pre_temper = -127000; //mtk_thermal_get_temp(MTK_THERMAL_SENSOR_CPU);
    tasklet_init(&host->testmode_tasklet, msdc_tasklet_testmode, (ulong)host);
#endif // MTK_SDIO30_TEST_MODE_SUPPORT
...
#ifdef MTK_SDIO30_TEST_MODE_SUPPORT
    init_timer(&host->testmode_timer);
    host->testmode_timer.function	= msdc_timer_testmode; 
    host->testmode_timer.data		= (unsigned long)host;
    host->testmode_timeout = 0;
#endif
...
#ifdef MTK_SDIO30_TEST_MODE_SUPPORT
    tasklet_kill(&host->testmode_tasklet);
#endif // MTK_SDIO30_TEST_MODE_SUPPORT
    if (mem)
