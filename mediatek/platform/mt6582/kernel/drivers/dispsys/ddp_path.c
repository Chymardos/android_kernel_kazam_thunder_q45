...
static int disp_reg_backup_module(DISP_MODULE_ENUM module);
static int disp_reg_restore_module(DISP_MODULE_ENUM module);
...
static void _disp_path_mutex_reg_update_cb(unsigned int param)
{
    ...
    if (param & (DDP_MUTEX_INTR_BIT << gMutexID)) {
    	...
    	// clear intr status
    	DISP_REG_SET(DISP_REG_CONFIG_MUTEX_INTSTA, 0xf0);
    	// dump reg info
    	disp_dump_reg(DISP_MODULE_CONFIG);
...
static int disp_path_ovl_reset()
{
...
static void disp_path_reset () {
	...
	while((DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON)&0x700)==0x100)
	{
		delay_cnt++;
		if(delay_cnt>10000)
		{
			printk("[DDP] error, RDMAReset(%d) timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x \n", 1, DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON));
			break;
		}
	}
...
int disp_check_engine_status(int mutexID)
{
    ...
    if(engine&DDP_RDMA1_UPDATE_BIT) //RDMA1
    {
        if((DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON)&0x1) ==0 || 
           (DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0)&DDP_RDMA1_POWER_BIT) != 0)
        {
            result = -1;
            DISP_ERR("rdma1 abnormal, en=%d, clk=0x%x \n", 
...
//    unsigned int cnt = 0;    
    ...
    //DISP_MSG("disp_path_release_mutex %d, %d \n", disp_mutex_unlock_cnt[mutexID]++, mutexID);
    ...
    g_dst_module = pConfig->dstModule;
       ...
       
        {
	        ...
	        else if(pConfig->srcModule==DISP_MODULE_RDMA1)
	        {
	            ...
	            DISP_REG_SET(DISP_REG_CONFIG_MUTEX_MOD(gMutexID), (1<<10)|(1<<7)|(1<<9)); //ovl, rdma1, color, bls
...
static int disp_reg_backup()
{
  ...
  reg_backup(DISP_REG_RDMA_FIFO_LOG          );
...
static int disp_reg_restore()
{
    ...
    reg_restore(DISP_REG_RDMA_FIFO_LOG          );
...
int disp_intr_restore(void)
{
    ...
    DISP_REG_SET(DISP_REG_WDMA_INTEN,      disp_intr_status[DISP_MODULE_WDMA0]); 
    DISP_REG_SET(DISP_REG_RDMA_INT_ENABLE,        disp_intr_status[DISP_MODULE_RDMA1]); 
...
int disp_intr_disable_and_clear(void)
{
    ...
    disp_intr_status[DISP_MODULE_RDMA1] = DISP_REG_GET(DISP_REG_RDMA_INT_ENABLE);
