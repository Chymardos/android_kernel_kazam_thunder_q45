...
#ifdef ENABLE_DCREMOVE
    mSwapBufferOne = new uint8_t[bufferSize()];
    ASSERT(mSwapBufferOne != NULL);
    mDcRemove = new DcRemove();
    ASSERT(mDcRemove != NULL);
    mDcRemove->init(mDL1Attribute->mChannels, mDL1Attribute->mSampleRate, DcRemove::DCR_MODE_3);
#endif
...
ssize_t AudioMTKStreamOut::WriteDataToBTSCOHW(const void *buffer, size_t bytes)
{
    ...
    do
    {
        ...
        do
        {
            ...
            ALOGVV("WriteDataToBTSCOHW, do encode outsize=%d, consumed size=%d, bytes=%d", outsize, insize, bytes);
            outbuffer += outsize;
            inbuf += insize;
            bytes -= insize;
            insize = bytes;
...
status_t AudioMTKStreamOut::SetAnalogFrequency(uint32 AfeDigital)
{
    ...
    switch (AfeDigital)
    {
        ...
        {
            mAudioResourceManager->SetFrequency(AudioResourceManagerInterface::DEVICE_OUT_DAC, mDL1Out->mI2S_SAMPLERATE);
            break;
        }
...
size_t AudioMTKStreamOut::WriteDataToAudioHW(const void *buffer, size_t bytes)
{
    ...
    else
    {
        ...
        {
            mFilters->start();
            outputSize = mFilters->process(inbuffer, bytes, outbuffer);
            if (outputSize == 0)
            {
    ...
    ret_ms = ::ioctl(mFd, AUDDRV_GET_DL1_REMAINDATA_TIME, NULL);
    struct timespec TimeInfo = GetSystemTime(false);
