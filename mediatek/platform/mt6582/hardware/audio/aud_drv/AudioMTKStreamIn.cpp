...
#define VOIP_PLATFORM_OFFSET_TIME (0)
    ...
    if (mStarting == false)
    {
        ...
        switch (mAudioResourceManager->GetAudioMode())
        {
            ...
            case AUDIO_MODE_IN_COMMUNICATION:
            {
                ...
                if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || mVoIPRunning || mHDRecTunning16K)
                {
...
void AudioMTKStreamIn::ConfigHDRecordParams(SPE_MODE mode)
{
    ...
    else    //default using recording one
    {
        ...
        if (mIsAPDMNRTuningEnable || ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) && ((QueryFeatureSupportInfo()& SUPPORT_ASR) > 0)))
        {
            for (int i = 0; i < NUM_ABFWB_PARAM; i++)
            {
...
void AudioMTKStreamIn::CheckHDRecordVoIPSwitch(void)
{
    ...
    {
        ALOGD("CheckHDRecordVoIPSwitch enter REC");
...
bool AudioMTKStreamIn::CheckVoIPNeeded(void)
{
    ...
    else
    {
        ...
        if (bNormalModeVoIP)
        {
            ...
            if ((mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION) || (mHDRecordSceneIndex != -1))
            {
                bRet = false;
            }
            else
