...
static const uint16_t kFmUplinkSamplingRateHz = 44100; // For MT6627, I2S FM only support 32000, but use ASRC to 44100, so that MEMIF will get I2S data with 44100Hz
static const uint16_t kFmDownlinkSamplingRateHz = kFmUplinkSamplingRateHz;
...
status_t AudioFMResourceManager::SetFmSourceModuleEnable(const bool enable)
{
    ...
    if (enable == true)
    {
        ...
        mAudioDigitalInstance->Set2ndI2SInConfig(kFmChipSamplingRateHz, bIsSlaveMode); // FM Rx I2S in from connsys is 32K / Master
        mAudioDigitalInstance->SetI2SASRCConfig(true, GetFmUplinkSamplingRate());  // Covert from 32000 Hz to 44100 Hz
    ...
    else
    {
        ...
        mAudioDigitalInstance->SetI2SASRCConfig(false, GetFmUplinkSamplingRate()); // Setting to bypass ASRC
