...
#if defined(BT_SW_CVSD) && !defined(EXTMD_LOOPBACK_TEST) //use VoIP to test. remark this to avoid VoIP MTKRecordThread controls AFE and kernel since they are controller by ExtMd threads
...
#if defined(BT_SW_CVSD) && !defined(EXTMD_LOOPBACK_TEST)
...
bool AudioMTKStreamInManager::AudioMTkRecordThread::threadLoop()
{
    ...
    while (!(exitPending() == true))
    {
...
#endif
        {
            ...
            //int ret_ms = ::ioctl(mFd, AUDDRV_GET_DL1_REMAINDATA_TIME, NULL);
            int ret_ms = 0;
            ...
            //ALOGD("tempTime sec= %ld, nsec=%ld, ret_ms=%d" , tempTime.tv_sec, tempTime.tv_nsec, ret_ms);
