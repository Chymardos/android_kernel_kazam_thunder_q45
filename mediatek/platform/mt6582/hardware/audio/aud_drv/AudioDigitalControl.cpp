...
{
#if 1
    ALOGW("MT6582 unsupport SetMrgI2SEnable");
#else
...
{
#if 1
    ALOGW("MT6582 unsupport SetDAIBBT");
#else
    ALOGD("+%s()\n", __FUNCTION__);
...
{
#if 1
    ALOGW("MT6582 unsupport GetDAIBTOut");
#else
    ALOGD("+%s()\n", __FUNCTION__);
...
{
#if 1
    ALOGW("MT6582 unsupport SetDAIBTEnable");
#else
    ALOGD("+%s(), bEnable = %d\n", __FUNCTION__, bEnable);
    ...
    if (bEnable == true)
    {
        ...
        ALOGD("SetDAIBTEnable AUDDRV_SET_BT_PCM_GPIO");
        ::ioctl(mFd, AUDDRV_SET_BT_PCM_GPIO);               //PCM_ON,  Analog FM
#else
        ALOGD("SetDAIBTEnable AUDDRV_SET_BT_FM_GPIO");
        ::ioctl(mFd, AUDDRV_SET_BT_FM_GPIO);                //PCM_ON, Digital FM
#endif

#if defined(MTK_MERGE_INTERFACE_SUPPORT)
        // When try to enable DAIBT
