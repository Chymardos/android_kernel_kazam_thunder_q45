...
//
/////////////////////////////////////////////////////////////////////////
MRESULT mrlscHWTBLCAL(TBL_INFO_T info)
{
    int kk, jj, ii,i;
    int m,n,nn;
...
//        ACDK_LOGD("[%s] get 12 coef\n", __FUNCTION__);
    ...
    //Range check, if coeff over-range, just clip to limit and show warning msg.
    /* limit a00 to 511,MT6589 and MT6582 support 511(511/128 gain), 
           but MT6571 and 6593 support to 1023( 1023/128 gain) */
        ...
        for(ii = 0; ii < m-1; ii++)
        {
            for(nbayer=0;nbayer<4;nbayer++)
            {
                for( kk=0; kk<12; kk++)
                {
                    ...
                    if(coef1<-511 || coef1>511) // early temination to recalcu coef in range
                    {
                        printf("ERROR !!\nRow %d, Column %d, Bayer %d, %dth Coeff value %d\n", jj, ii, nbayer, kk, coef1);
                        if(kk==11)
                        {
                            ACDK_LOGD("Coef A00value exceed register range, Please reduce destination gain !\n");
                        }
                        else
                        {
                            ACDK_LOGD("Coef value exceed register range, Please contact Shading Owner !\n");
                        }
                        coef[nbayer][jj*(m-1)+ii][kk] = (coef1<-511)? -511: 511;
                        mrRet = E_CCT_CALIBRATION_COEF_OVERFLOW;
                    }
                }             
            }
        }
...
//#include "float-gain_table.txt"
    ...
    /*Shading Link Parameter*/
    float gain_inverse[4];
    float Centor_x = 0.0;
    float Centor_y = 0.0;
    float max_gain_inverse = 0.0;
    float sum_max_gain_inverse = 0.0;
...
// debug
    ...
    sprintf(fileName, "%s//sl2.dat", MEDIA_PATH);
    FILE *finsl2;
    finsl2  = fopen(fileName, "wt");
#endif
    for(j = 0; j < n; j++)
    {
        for(i = 0; i < m; i++)
        {
            ...
            for(k=0;k<4;k++)
            {
                ...
                /* shading link strenght ~ gain inverse  */
                if(val[k])
                {
                    gain_inverse[k] = 1.0/(float)val[k];
                    /* find the max of inverse gain among 4 channels */
                    if (max_gain_inverse < gain_inverse[k])
                        max_gain_inverse = gain_inverse[k];  
                }
                else
                {
                    mrRet = E_CCT_CALIBRATION_COEF_OVERFLOW;
            ...
            Centor_x += kx*max_gain_inverse;
            Centor_y += ky*max_gain_inverse;
            sum_max_gain_inverse += max_gain_inverse;
    ...
    tbl_cal_info.sl2_setting.SL2_CENTR_X   = Centor_x/sum_max_gain_inverse;
    tbl_cal_info.sl2_setting.SL2_CENTR_Y   = Centor_y/sum_max_gain_inverse;
    tbl_cal_info.sl2_setting.SL2_RR_0      = (g_lsc_param.plane_wd/2*0.3)*(g_lsc_param.plane_wd/2*0.3) + (g_lsc_param.plane_ht/2*0.3)*(g_lsc_param.plane_ht/2*0.3);
    tbl_cal_info.sl2_setting.SL2_RR_1      = (g_lsc_param.plane_wd/2*0.6)*(g_lsc_param.plane_wd/2*0.6) + (g_lsc_param.plane_ht/2*0.6)*(g_lsc_param.plane_ht/2*0.6);
    tbl_cal_info.sl2_setting.SL2_RR_2      = (g_lsc_param.plane_wd/2*0.8)*(g_lsc_param.plane_wd/2*0.8) + (g_lsc_param.plane_ht/2*0.8)*(g_lsc_param.plane_ht/2*0.8);
    tbl_cal_info.sl2_setting.regsetting[0] = ((((unsigned int)(tbl_cal_info.sl2_setting.SL2_CENTR_Y) << 16) & 0x0fff0000) | 
                                               ((unsigned int)(tbl_cal_info.sl2_setting.SL2_CENTR_X)        & 0x00001fff));
    tbl_cal_info.sl2_setting.regsetting[1] = ((unsigned int)tbl_cal_info.sl2_setting.SL2_RR_0 & 0x01ffffff );
    tbl_cal_info.sl2_setting.regsetting[2] = ((unsigned int)tbl_cal_info.sl2_setting.SL2_RR_1 & 0x01ffffff );
    tbl_cal_info.sl2_setting.regsetting[3] = ((unsigned int)tbl_cal_info.sl2_setting.SL2_RR_2 & 0x01ffffff );

#if LSC_Debug_Table_Output
    fprintf(finsl2, "Centor_x = %f\n",tbl_cal_info.sl2_setting.SL2_CENTR_X);
    fprintf(finsl2, "Centor_y = %f\n",tbl_cal_info.sl2_setting.SL2_CENTR_Y);
    fprintf(finsl2, "SL2_RR0 = %f\n", tbl_cal_info.sl2_setting.SL2_RR_0);
    fprintf(finsl2, "SL2_RR1 = %f\n", tbl_cal_info.sl2_setting.SL2_RR_1);
    fprintf(finsl2, "SL2_RR2 = %f\n", tbl_cal_info.sl2_setting.SL2_RR_2);
    fprintf(finsl2, "\n");
    fprintf(finsl2, "0x%08x, ", tbl_cal_info.sl2_setting.regsetting[0]);
    fprintf(finsl2, "0x%08x, ", tbl_cal_info.sl2_setting.regsetting[1]);
    fprintf(finsl2, "0x%08x, ", tbl_cal_info.sl2_setting.regsetting[2]);
    fprintf(finsl2, "0x%08x  ", tbl_cal_info.sl2_setting.regsetting[3]);
