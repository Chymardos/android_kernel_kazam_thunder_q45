...
void FC_init(AcdkBase* p)
{
	...
	if(ini==1)
	{
		...
		g_baseObj->sendcommand(ACDK_CMD_PREVIEW_START, (UINT8 *)&rCCTPreviewConfig, sizeof(ACDK_CCT_CAMERA_PREVIEW_STRUCT), (MUINT8 *)0, 0, &u4RetLen22);
		usleep(5000000);
		//------------------------
	}
...
MUINT32 u4RetLen = 0;
	...
	//FlashMgr::getInstance()->cctSetCapDutyStep(para->isFlash, para->duty, para->step); //YosenFlash
    ...
    int ts, te;
    //FlashMgr::getInstance()->cctGetLastFireTime(&ts, &te); //YosenFlash
