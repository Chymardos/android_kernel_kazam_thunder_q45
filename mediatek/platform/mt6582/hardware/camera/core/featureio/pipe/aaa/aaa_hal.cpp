...
MINT32 Hal3A::sm_3AThreadLogEnable = 0;
...
init(MINT32 i4SensorDev)
{
	...
	IState::sm_3APvLogEnable = sm_3AThreadLogEnable = atoi(value);
...
{
	MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] enter, eCmd = %d\n", eCmd);
	...
	MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] before StateMgr::sendCmd(%d)\n", eCmd);
    ERROR_CHECK(StateMgr::getInstance().sendCmd(eCmd))
	MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] after StateMgr::sendCmd(%d)\n", eCmd);
    ...
    if (eCmd == ECmd_Update)
    {
        ...
        MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] IspTuningMgr::validatePerFrame(MFALSE) done\n");


        if (g_semAFIRQWait == 0)   {
            ::sem_post(&g_semAFThreadstart);
            MY_LOG_IF(sm_3AThreadLogEnable, "[AFThread] sem_post g_semAFThreadstart\n");
        }
        ...
        if((mpFlickerHal!=NULL) )
        {
            ...
            if(m_rParam.u4AntiBandingMode==AE_FLICKER_MODE_AUTO/*2*/)
            {
				...
				MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] mpFlickerHal Enable & Update done\n");
            }
            else
            {
                ERROR_CHECK(mpFlickerHal->sendCommand(FLKCmd_FlkDISable,NULL));
				MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] mpFlickerHal disable done\n");
            }
        }
        CPTLog(Event_Pipe_3A_Flicker, CPTFlagEnd);    // Profiling End.

    }
	MY_LOG_IF(sm_3AThreadLogEnable,"[Hal3A::postCommand] exit, eCmd = %d\n", eCmd);
    return MTRUE;
}
...
MVOID * Hal3A::AFThreadFunc(void *arg)
{
    ...
    while (g_bAFThreadLoop) {
        ...
        MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] sem_wait g_semAFThreadstart done\n");
        if (g_bAFThreadLoop != 0)
        {
            AfMgr::getInstance().DoCallback();
			MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] AfMgr::DoCallback() done\n");
            if (g_pIspDrv->waitIrq(WaitIrq) > 0) // success
            {
                MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] AF waitIrq done\n");
                StateMgr::getInstance().sendCmd(ECmd_AFUpdate);
				MY_LOG_IF(sm_3AThreadLogEnable, "[Hal3A::AFThreadFunc] StateMgr::sendCmd(ECmd_AFUpdate) done\n");
            }
            else
            {
