...
/*
    	...
    	*/
    	...
    	if(skipFrames>0)
    	{
			...
			//modify JWLW-373 zhaolong 20130918 (start)
			//if((FlashMgr::getInstance()->getFlashMode() != FLASHLIGHT_TORCH) && (skipFrames == AfLampOnSkipFrame-2))
			if((FlashMgr::getInstance()->getFlashMode() != FLASHLIGHT_TORCH) && (skipFrames == AfLampOnSkipFrame-2) 
				&& (g_ePrevState != eState_Recording))
			//modify JWLW-373 zhaolong 20130918 (end)
			{
	...
	else if((FlashMgr::getInstance()->isAFLampOn()==1) && (FlashMgr::getInstance()->getFlashMode() != FLASHLIGHT_TORCH)  && (g_isPostPrecapAF==0)
		&& (g_ePrevState != eState_Recording))
	//modify JWLW-373 zhaolong 20130918 (end)
        {
