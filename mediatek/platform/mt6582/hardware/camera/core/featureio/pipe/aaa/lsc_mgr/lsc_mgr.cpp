...
getCTIdx() {
    ...
    MY_LOG_IF(_bEnableMyLog, "[%s] Shading idx= %d\n", __FUNCTION__, m_u4CTIdx);
    return m_u4CTIdx;
}
...
setCTIdx(MUINT32 const u4CTIdx) {
    ...
    if (dbg_ct != -1) {
        ...
        if (isTSFEnable() == MTRUE && m_bMetaMode != MTRUE) {
            ...
            MY_LOG_IF(_bEnableMyLog, "[%s] TSF ading idx = %d\n", __FUNCTION__, m_u4CTIdx);
        } else {
...
SetTBAToISP()     //  ISP <- PA
{
    ...
    RawLscTblFlushCurrTbl();
#if ENABLE_TSF
    MY_LOG_IF(_bEnableMyLog, "[LscMgr:%s] TSF enable %d, state %d", __FUNCTION__,
            isTSFEnable(),
            mTSFState);
    if (isTSFEnable() == MTRUE && mTSFState != LSCMGR_TSF_STATE_IDLE)
    {
        ...
        MY_LOG_IF(_bEnableMyLog, "[LscMgr:%s] upate TSF phy addr 0x%08x, vir addr 0x%08x, size 0x%08x", __FUNCTION__,
...
#endif  //ENABLE_TSF
    {
        ...
        MY_LOG_IF(_bEnableMyLog, "[LscMgr] %s m_eLscScenario %d, TableSize %d, CCT 0x%x, phyAddr 0x%x, XSize 0x%x m_eSensorOp %d\n",
                __FUNCTION__,
                m_eLscScenario,
...
#if ENABLE_TSF
,
m_bTSF(MFALSE), m_bTsfForceAWB(MFALSE)
#endif
{
...
#else
    MUINT32 i4AddrTbl   = stRawLscInfo[eLscScn].virtAddr+getCTIdx()*getPerLutSize(eLscScn);
#endif
...
prepareTSFInputBuffer(EIspProfile_T profile, LSCMGR_TSF_STATE state)
{
    ...
    {
        MUINT32 i4AddrCap = (stRawLscInfo[lsc_scenario].virtAddr+getCTIdx()*getPerLutSize(lsc_scenario));
        MUINT32 i4Size = getPerLutSize(lsc_scenario);
        MY_LOG("[LscMgr:%s] Normal Capture, Copy CapTbl(0x%08x) to input(0x%08x), size(%d)\n", __FUNCTION__,
            i4AddrInput, i4AddrCap, i4Size);
        memcpy((void*)i4AddrInput, (void*)i4AddrCap, i4Size);
    }
    else
    {
        if (!(EIspProfile_NormalPreview == profile))
        {
            ...
            {
                //if (profile == EIspProfile_ZsdPreview_NCC)
                //    lsc_scenario = LSC_SCENARIO_01;
                
            #if USING_BUILTIN_LSC
                MUINT32 i4AddrTbl   = getLut(lsc_scenario);
            #else
                MUINT32 i4AddrTbl   = stRawLscInfo[lsc_scenario].virtAddr+getCTIdx()*getPerLutSize(lsc_scenario);
            #endif
                MUINT32 i4PrevSize  = getPerLutSize(m_ePrevLscScenario);
                MUINT32 i4NewSize   = getPerLutSize(lsc_scenario);
                MY_LOG("[LscMgr:%s] Need BAK from input(0x%08x) to bak(0x%08x), size(%d)\n", __FUNCTION__,
                        i4AddrInput, i4AddrBak, i4PrevSize);
                memcpy((void*)i4AddrBak, (void*)i4AddrInput, i4PrevSize);

                MY_LOG("[LscMgr:%s] from Tbl(0x%08x) to input(0x%08x), size(%d)\n", __FUNCTION__,
                        i4AddrTbl, i4AddrInput, i4NewSize);
                memcpy((void*)i4AddrInput, (void*)i4AddrTbl, i4NewSize);
            }
        } 
        else
        {
            MY_LOG("[LscMgr:%s] EIspProfile_NormalPreview, from bak(0x%08x) to input(0x%08x)\n", __FUNCTION__,
                    m_TSFBuff[TSF_BUFIDX_BAK].virtAddr, m_TSFBuff[TSF_BUFIDX_INPUT].virtAddr);
            memcpy((void*)i4AddrInput, (void*)i4AddrBak, m_TSFBuff[TSF_BUFIDX_BAK].size);
        }
    }
#else
    ...
    #if 0
    if (m_ePrevIspProfile == EIspProfile_NormalPreview)
    {
        if (profile != EIspProfile_NormalPreview)
        {
            if (profile == EIspProfile_ZsdPreview_NCC || profile == EIspProfile_NormalCapture)
                lsc_scenario = LSC_SCENARIO_01;
                
            if (state == LSCMGR_TSF_STATE_SCENECHANGE)
            {
                MY_LOG("[%s] SCENECHANGE: profile(preview => %d), backup", __FUNCTION__, profile);
                backupTSFTbl();
                MY_LOG("[%s] SCENECHANGE: profile(preview => %d), assign", __FUNCTION__, profile);
                assignTSFTbl(lsc_scenario);
            }
            else
            {
                //MY_LOG("[%s] GETNEWINPUT: profile(preview => %d), assign", __FUNCTION__, profile);
                //assignTSFTbl(lsc_scenario);
            }
        }
    }
    else
    {
        if (profile == EIspProfile_NormalPreview)
        {
            MY_LOG("[%s] state (%d): profile(%d => preview), restore", __FUNCTION__, state, m_ePrevIspProfile);
            restoreTSFTbl();
        }
        else
        {
            MY_LOG("[%s] state (%d): profile(%d => %d), assign", __FUNCTION__, state, m_ePrevIspProfile, profile);
            assignTSFTbl(lsc_scenario);
        }
    }
    #endif
