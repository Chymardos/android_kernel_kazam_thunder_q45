...
MRESULT AeMgr::cameraPreviewInit(MINT32 i4SensorDev, Param_T &rParam)
{
    ...
    {
        m_eAECamMode = LIB3A_AECAM_MODE_PHOTO;
        MY_LOG("m_eAECamMode = LIB3A_AECAM_MODE_PHOTO;");
    }
...
MRESULT AeMgr::cameraPreviewReinit()
{
    ...
    if(m_bRealISOSpeed == 1) {
        MY_LOG("[cameraPreviewReinit] Real ISO:%d Exp mode: %d, Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n",
                    m_bRealISOSpeed, g_rAEOutput.rPreviewMode.u4ExposureMode, g_rAEOutput.rPreviewMode.u4Eposuretime,
...
MUINT32 u4DFpsThres = 0;
    ...
    if(m_bEnableAE) {
        ...
        if(m_i4WaitVDNum == 0x00) {
            ...
            if(m_eZoomWinInfo.bZoomChange == MTRUE) {
              ...
              //  copyAEInfo2mgr(&g_rAEOutput.rPreviewMode, &rAEOutput);
                ...
                if(m_eAECamMode == LIB3A_AECAM_MODE_VIDEO) {
                    // Check the dynamic frame rate enable or not.
                    if(g_rAEInitInput.rAEPARAM.strVdoDFps.isEnableDFps == MTRUE) {
                        if(m_eAEMode == LIB3A_AE_MODE_NIGHT) {
                            u4DFpsThres = g_rAEInitInput.rAEPARAM.strVdoDFps.EVThresNight;
                        } else {
                            u4DFpsThres = g_rAEInitInput.rAEPARAM.strVdoDFps.EVThresNormal;
                        }

                        if(u4DFpsThres > getLVvalue()) {
                            if(m_bVideoDynamic == MFALSE) {
                                m_u4DynamicFrameCnt++;
                                if(m_u4DynamicFrameCnt > DYNAMIC_FRAME_COUNT) {
					...
					MY_LOG("[doPvAE]m_bVideoDynamic:%d  getLVvalue:%d\n", m_bVideoDynamic,getLVvalue());
            ...
            if(m_pIAeAlgo != NULL) {
                MY_LOG("[doPvAE]AE Wait Vd frame:%d Enable:%d avgY:%d\n", m_i4WaitVDNum, m_bEnableAE, m_pIAeAlgo->getBrightnessAverageValue());
            }
...
{
	MY_LOG("setRestore line=%d frmm=%d\n", __LINE__, frm);
	int maxFrmDelay=0;
	...
	else if(frm==1+expSetFrm) //off exp
	{
		...
		AAASensorMgr::getInstance().setPreviewMaxFrameRate(frmRate);
#else
#endif
	...
	{
#ifdef MTK_AF_SYNC_RESTORE_SUPPORT
		AAASensorMgr::getInstance().setPreviewMaxFrameRate(g_defaultFrameRate);
#else
#endif
		setExp(g_rExp);
...
MRESULT AeMgr::copyAEInfo2mgr(AE_MODE_CFG_T *sAEOutputInfo, strAEOutput *sAEInfo)
{
	...
	if(m_bStrobeOn != MTRUE) {
    	m_BVvalue = sAEInfo->Bv;
    } else {
        MY_LOG("[copyAEInfo2mgr] Strobe on :%d and Skip BV value update:%d\n", m_bStrobeOn, m_BVvalue);
    }
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    MY_LOG("AE max block width:%d heigh:%d AE mter:%d CamMode:%d AEMode:%d Flicker :%d %d EVcomp:%d ISO:%d %d MinFps:%d MaxFps:%d Limiter:%d\n", g_rAEInitInput.i4AEMaxBlockWidth, g_rAEInitInput.i4AEMaxBlockHeight, m_eAEMeterMode, m_eAECamMode, m_eAEMode, m_eAEFlickerMode, m_eAEAutoFlickerMode, m_eAEEVcomp, m_eAEISOSpeed, m_bRealISOSpeed, m_i4AEMinFps, m_i4AEMaxFps, m_bAElimitor);
    m_pIAeAlgo = IAeAlgo::createInstance();
    if (!m_pIAeAlgo) {
...
{
    g_rAEOutput.rPreviewMode = a_rPreviewInfo;
    ...
    MY_LOG("[updatePreviewParams] i4AEidxNext:%d Exp. mode = %d Preview Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n",
        i4AEidxNext, g_rAEOutput.rPreviewMode.u4ExposureMode, g_rAEOutput.rPreviewMode.u4Eposuretime,
        g_rAEOutput.rPreviewMode.u4AfeGain, g_rAEOutput.rPreviewMode.u4IspGain, g_rAEOutput.rPreviewMode.u2FrameRate, g_rAEOutput.rPreviewMode.i2FlareGain, g_rAEOutput.rPreviewMode.i2FlareOffset, g_rAEOutput.rPreviewMode.u4RealISO);
...
MRESULT AeMgr::UpdateSensorISPParams(AE_STATE_T eNewAEState)
{
    ...
    switch(eNewAEState)
    {
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            ...
            g_rAEOutput.rPreviewMode.i2FlareOffset=rAEInfo2ISP.i2FlareOffset;
            g_rAEOutput.rPreviewMode.i2FlareGain=FLARE_SCALE_UNIT * FLARE_OFFSET_DOMAIN / (FLARE_OFFSET_DOMAIN - rAEInfo2ISP.i2FlareOffset);
...
//            MY_LOG("[doPvAE] Flare Gain:%d Flare Offset:%d\n", g_rAEOutput.rPreviewMode.i2FlareGain, g_rAEOutput.rPreviewMode.i2FlareOffset);
...
MRESULT AeMgr::getBrightnessValue(MBOOL * bFrameUpdate, MINT32* i4Yvalue)
{
    if(m_bFrameUpdate == MTRUE) {
        if(m_pIAeAlgo != NULL) {
            * i4Yvalue = m_pIAeAlgo->getBrightnessAverageValue();
        } else {
            MY_LOG("The AE algo class is NULL (40)\n");
        }
