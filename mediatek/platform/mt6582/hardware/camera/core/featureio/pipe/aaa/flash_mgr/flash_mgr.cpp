...
int g_sceneCnt=0;
static volatile int g_previewMode=e_NonePreview;
...
FLASH_PROJECT_PARA& FlashMgr::getFlashProjectPara(int aeMode)
{
    ...
    //loadNvram();
	FLASH_PROJECT_PARA para;
if(m_sensorType==DUAL_CAMERA_MAIN_SENSOR)
	return cust_getFlashProjectPara(aeMode, pNv);
else
    return cust_getFlashProjectPara_sub(aeMode, pNv);
}
FlashMgr::FlashMgr()
{
	g_plineEvSetting=0;
...
void FlashMgr::hw_setPreference(FlashAlg* pStrobeAlg, FLASH_PROJECT_PARA* pPrjPara)
{
	...
	int i4SceneLV = AeMgr::getInstance().getLVvalue();
	int overAdd=0;
	int fbAdd=0;
	if(i4SceneLV<10)
		overAdd=3;
...
int FlashMgr::setFlashMode(int mode)
{
    ...
    else if(g_previewMode==e_VideoRecording)
    {
		...
		{
			LogInfo("VideoR XX");
            if(fmode != LIB3A_FLASH_MODE_AUTO) //add JWLW-373 zhaolong When flash mode auto, flash off when recording.
            turnOffFlashDevice();
        }
        else
        {
