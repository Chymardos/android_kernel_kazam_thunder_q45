...
// sym add for comptible with same IC different module but same lens in 10.25 2013 start
	MY_LOG("sym [lens][SensorDev]0x%04x, [SensorId]0x%04x, [CurrLensId]0x%04x", m_i4CurrSensorDev, m_i4CurrSensorId, m_i4CurrLensId);
	if( 0x8824 == m_i4CurrSensorId)
		m_i4CurrSensorId = 0x8825;
	//sym end
...
MRESULT AfMgr::setFDInfo(MVOID* a_sFaces)
{
    ...
    sFDArea.rAreas[0].i4Left   = (pFaces->faces->rect[0]+1000) * m_sAFInput.sEZoom.i4W / 2000 + m_sAFInput.sEZoom.i4X;
    sFDArea.rAreas[0].i4Right  = (pFaces->faces->rect[2]+1000) * m_sAFInput.sEZoom.i4W / 2000 + m_sAFInput.sEZoom.i4X;
    sFDArea.rAreas[0].i4Top    = (pFaces->faces->rect[1]+1000) * m_sAFInput.sEZoom.i4H / 2000 + m_sAFInput.sEZoom.i4Y;
    sFDArea.rAreas[0].i4Bottom = (pFaces->faces->rect[3]+1000) * m_sAFInput.sEZoom.i4H / 2000 + m_sAFInput.sEZoom.i4Y;
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if (m_pMcuDrv)  {        
         ...
         if(preMCUpos!=m_sAFOutput.i4AFPos)
             m_pMcuDrv->moveMCU(m_sAFOutput.i4AFPos);
         preMCUpos=m_sAFOutput.i4AFPos;
    }
    else   {
...
MVOID AfMgr::setFlkWinConfig()
{
    ...
    if( (m_PreAFArea.sRect[0].i4X !=i4X) ||( m_PreAFArea.sRect[0].i4W !=i4W) 
     || (m_PreAFArea.sRect[0].i4Y !=i4Y) || (m_PreAFArea.sRect[0].i4H !=i4H) || (m_PreFDArea.i4Count !=m_FDArea.i4Count))
    {
        m_PreAFArea.sRect[0].i4X =i4X;
        m_PreAFArea.sRect[0].i4W =i4W; 
        m_PreAFArea.sRect[0].i4Y =i4Y;
        m_PreAFArea.sRect[0].i4H =i4H; 
        m_PreFDArea.i4Count =m_FDArea.i4Count;
    ...
    if (m_FDArea.i4Count)   {
        ...
        if(   (m_PreFDArea.sRect[0].i4X !=i4XE) || (m_PreFDArea.sRect[0].i4Y !=i4YE) 
           || (m_PreFDArea.sRect[0].i4W !=m_FDArea.sRect[0].i4W) || (m_PreFDArea.sRect[0].i4H !=m_FDArea.sRect[0].i4H) )
        {
        
            m_PreFDArea.sRect[0].i4X =i4XE;
            m_PreFDArea.sRect[0].i4Y =i4YE; 
            m_PreFDArea.sRect[0].i4W =m_FDArea.sRect[0].i4W;
            m_PreFDArea.sRect[0].i4H =m_FDArea.sRect[0].i4H; 
...
MVOID AfMgr::setAFWinTH(AF_CONFIG_T a_sAFConfig)
{
    ...
    // 8 bits
    if (m_i4AF_THEX != a_sAFConfig.AF_THEX)   {
        MY_LOG("[setAFWinTHEX]%d \n", a_sAFConfig.AF_THEX);        
        ISP_WRITE_REG(m_pIspReg , CAM_AF_TH_E, Boundary(0, a_sAFConfig.AF_THEX, 255));
        m_i4AF_THEX = a_sAFConfig.AF_THEX;
    }
}
...
MVOID AfMgr::setGMR(AF_CONFIG_T a_sAFConfig)
{
   ...
   // if ((m_i4GMR[0] != a_sAFConfig.i4SGG_GMR1) || (m_i4GMR[1] != a_sAFConfig.i4SGG_GMR2) || (m_i4GMR[2] != a_sAFConfig.i4SGG_GMR3))   {
        ...
        MY_LOG("[setGMR][m_i4GMR] %d %d %d [a_sAFConfig] %d %d %d", m_i4GMR[0], m_i4GMR[1], m_i4GMR[2], a_sAFConfig.i4SGG_GMR1, a_sAFConfig.i4SGG_GMR2, a_sAFConfig.i4SGG_GMR3);
...
MVOID AfMgr::setAFConfig(AF_CONFIG_T a_sAFConfig)
{
    ...
    ISP_WRITE_BITS(m_pIspReg , CAM_AF_CON, AF_TARX, 3);	//0: even
    ISP_WRITE_BITS(m_pIspReg , CAM_AF_CON, AF_TARY, 3);	//0: even
...
MVOID AfMgr::printAFConfigLog1()
{
    ...
    MY_LOG("[WinXE]%d [WinYE]%d [SizeXE]%d [SizeYE]%d [THE]%d [AFMode]%d",
...
AF_STAT_T AfMgr::Trans4WintoOneStat(MVOID *pAFStatBuf)
{
    ...
    if(m_tcaf_mode && !m_FDArea.i4Count)    {
