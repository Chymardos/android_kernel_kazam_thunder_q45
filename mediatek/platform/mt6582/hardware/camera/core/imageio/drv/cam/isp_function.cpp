...
//TEST 0418            ISP_WRITE_REG(m_pPhyIspReg ,CAM_CTL_TILE, 0x7A03001E);  // TEST_MDP // hardware do not read both tpipe width and height
    ...
    if(m_pIspDrv->IsReadOnlyMode())
    {
        ...
        // [mdpmgr] to sperate pass2 done and pass2B done
        //remove to dpframework control: ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspDrv->getRegAddrMap(), CAM_CTL_PIX_ID_SET, ISP_FRAME_DONE_SEL_SET, 0x1);
        //Make MDP direct-link form YUV(1), not CDRZ(0)
...
//TEST 0418        ISP_WRITE_BITS(m_pIspReg, CAM_CTL_PIX_ID_SET, CDP_EDGE_SET, 0xE); // TEST_MDP
        // [mdpmgr] to sperate pass2 done and pass2B done
        //remove to dpframework control: ISP_WRITE_BITS(m_pIspReg, CAM_CTL_PIX_ID_SET, ISP_FRAME_DONE_SEL_SET, 0x1);
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    else if (ISP_PASS2 == this->path) 
    {
        ...
        ISP_FUNC_INF("pass2 CQ1 g_pMdpMgr->stopMdp()>");
    ...
    else if (ISP_PASS2B == this->path) 
    {
        ...
        ISP_FUNC_INF("pass2 CQ2 g_pMdpMgr->stopMdp()>");
