...
public:
    ...
    bool init(ACDK_SCENARIO_ID_ENUM scenarioId) 
    {
        ...
        IhwScenario* pHwScenario = IhwScenario::createInstance(
                                                    ...
                                                    meSensorDev,
                                                    mSensorBitOrder); 
        if (ACDK_SCENARIO_ID_CAMERA_ZSD == scenarioId)
        {
            MY_LOGD("ZSD mode pass getHwValidSize");
        }
        else
        {
...
PreviewCmdQueThread::PreviewCmdQueThread(sp<IPreviewBufMgrHandler> pHandler, 
    ...
    , meCamPreviewEnd(ECmd_CameraPreviewEnd)
{
...
start()
{
    ...
    mp3AHal->setZoom(100, 0, 0, mSensorInfo.getImgWidth(), mSensorInfo.getImgHeight());
    {
        ...
        {
            case '4': // 4: "Video Preview Mode"
                if (1 == getParamsMgr()->getInt(MtkCameraParameters::KEY_ENG_ZSD_ENABLE))
                {
                    // NS3A::AeMgr::getInstance().setAECamMode(LIB3A_AECAM_MODE_ZSD);
                    mp3AHal->setIspProfile(EIspProfile_ZsdPreview_CC);
                    MY_LOGD("EIspProfile_ZsdPreview_CC");
                ...
                meCamPreviewEnd = ECmd_CamcorderPreviewEnd;
                MY_LOGD("ECmd_CamcorderPreviewStart");
                break;
            case '0': // 0: do not save
            case '1': // 1: "Preview Mode", 
            case '2': // 2: "Capture Mode",
            ...
            default:
                ...
                {
                    // NS3A::AeMgr::getInstance().setAECamMode(LIB3A_AECAM_MODE_ZSD);
                    // NS3A::AeMgr::getInstance().setAEMinMaxFrameRate(15000, 15000);
                    mp3AHal->setIspProfile(EIspProfile_ZsdPreview_CC);
                    MY_LOGD("EIspProfile_ZsdPreview_CC");
                ...
                meCamPreviewEnd = ECmd_CameraPreviewEnd;
                MY_LOGD("ECmd_CameraPreviewStart");
                break;
        }
    }
...
#undef TRUE
    ...
    // (4) Flash
    {
        ...
        FlashMgr::getInstance()->egSetMfDutyStep(duty, MtkCameraParameters::KEY_ENG_FLASH_STEP_DEFAULT_VALUE);
    }
...
stop()
{
    ...
    //(1) notify sw  
    mp3AHal->sendCommand(meCamPreviewEnd);
    MY_LOGD("mp3AHal->sendCommand(%d)", meCamPreviewEnd);
...
updateCheck()
{
    ...
    mspParamsMgr->updatePreviewFPS(rt_FPS);
