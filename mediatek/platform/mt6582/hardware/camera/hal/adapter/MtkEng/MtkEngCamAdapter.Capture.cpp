...
#warning "[TODO] Magic number: need to enumerize"
    ...
    int zsd_enable = pParamsMgr->getInt(MtkCameraParameters::KEY_ENG_ZSD_ENABLE);
    if (1 == zsd_enable)
    {
        engParam.mi4EngSensorMode = EngParam::ENG_SENSOR_MODE_ZSD;
    }
...
#warning "[TODO] perform a dummy compressed-image callback or CAMERA_MSG_ERROR to inform app of end of capture?"
    ...
    else
    {
        ...
        pParamsMgr->set(MtkCameraParameters::KEY_ENG_CAPTURE_ISO, engParam.u4EngCaptureISO);
    }
    //
    //
lbExit:
