...
my $mcddll_dll_file = "${input_path}/mtk_rel/${projectNoFlavorUC}/${flavorUC}/tst/database/mcddll.dll";
my $modem_elf_file = `find ${input_path}/build/${projectNoFlavorUC}/${flavorUC}/bin/ -name "*_PCB01_*.elf"`;
...
if ($debugFlag == 1) {
	...
	print "[DEBUG] \$modem_elf_file = $modem_elf_file\n";
}
...
my $elfinfo_file_name = &Get_Basename ("$modem_elf_file");
...
my %mapping_table = (
	...
	"$modem_elf_file" => "${elfinfo_file_name}${suffix}.elf",
);
