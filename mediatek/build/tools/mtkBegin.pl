...
&p_system("cp -r vendor/ragentek/${prj}/framework/*.apk ${projectout}/system/framework");
&p_system("cp -r vendor/ragentek/${prj}/framework/*.jar ${projectout}/system/framework");
# do not add 3rd application when use CTA
if ($ENV{"MTK_CTA_SUPPORT"} eq "no")
{
...
sub signApk {
  ...
  if ($ENV{"MTK_SIGNATURE_CUSTOMIZATION"} eq "yes")
  {
    ...
    {
      #modify renqingyou start
      #$keypath = "build/target/product/security/$prj";
      $keypath = "build/target/product/security/common";
      #modify renqingyou end
    }
  }
  else
  {
