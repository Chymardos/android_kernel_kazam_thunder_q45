...
my $PARTITION_DEFINE_C_NAME		= "mediatek/kernel/drivers/dum-char/partition_define.c";
	...
	for($index=0;$index < $total_rows;$index++){
		if($TYPE_FIELD[$index] eq "EXT4" || $TYPE_FIELD[$index] eq "FAT"){
			...
			if(exists($PSalias{$PARTITION_FIELD[$index]})){
				print EMMC_PART_SIZE_LOCATION "BOARD_$PSalias{$PARTITION_FIELD[$index]}IMAGE_PARTITION_SIZE:=$temp\n" ;
			}else{
				print EMMC_PART_SIZE_LOCATION "BOARD_$PARTITION_FIELD[$index]IMAGE_PARTITION_SIZE:=$temp\n" ;
			}
		}
	}
