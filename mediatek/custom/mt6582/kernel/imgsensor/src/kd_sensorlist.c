...
static int istsfopen = 0;
static ssize_t  tsf_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf){			
			     return sprintf(buf, "%d\n",istsfopen);
}

struct kobject *tsf_kobj;
//static int is_fwupd_success;
static struct kobj_attribute tsf_attr = {
	.attr = {
		.name = "tsf_distinguish",
		 .mode =  S_IRUGO,
...
#endif
//zhoulidong add for camera information (start) 
static char rgk_camera_back[32]= "NoBackCamera";
static char rgk_camera_front[32] = "NoFrontCamera";
static int rgk_read_camera_info(char *page, char **start, off_t off,int count,
			  int *eof, void *data){
	ssize_t len = 0;
	if(off > 0)
	{
	...
	len = (ssize_t)sprintf(page, "BackCamera:%s,FrontCamera:%s\n", rgk_camera_back,rgk_camera_front);
	
	return len;

}
//zhoulidong add for camera information (end) 
/*******************************************************************************
* Proifling
********************************************************************************/
#define PROFILE 1
...
//zhoulidong add (start)
		        else {

				if(DUAL_CAMERA_SUB_SENSOR==g_invokeSocketIdx[i]){
				   
					strncpy(rgk_camera_front,(char*)g_invokeSensorNameStr[i],sizeof(rgk_camera_front)-1);
					PK_DBG("%scall front rgk_camera_front %s \n", __func__,rgk_camera_front);
				
				}else{
					strncpy(rgk_camera_back,(char*)g_invokeSensorNameStr[i],sizeof(rgk_camera_back)-1);
					PK_DBG("%s call back rgk_camera_back = %s \n", __func__,rgk_camera_back);
					
					/* sym add  for tsf distinguish*/
					if(0x8825 == sensorID){
						istsfopen = 1;
						printk("sym istsfopen 1\n");
					}
...
inline static int RegisterCAMERA_HWCharDrv(void)
{
	...
	tsf_kobj =kobject_create_and_add("symdebug", NULL);
	err = sysfs_create_file(tsf_kobj, &tsf_attr.attr);
...
/*******************************************************************************
  ...
  * Used for sensor register read/write by proc file
  ********************************************************************************/
static int  CAMERA_SUBHW_Reg_Debug( struct file *file, const char *buffer, unsigned long count,
                                                                     void *data)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    prEntry = create_proc_entry("driver/subsensor", 0, NULL);
    if (prEntry) {
        prEntry->read_proc = CAMERA_HW_DumpReg_To_Proc;
        prEntry->write_proc = CAMERA_SUBHW_Reg_Debug;
    }
    else {
        PK_ERR("add /proc/driver/camsensor entry fail \n");
    }
 ...
 //zhoulidong  add it for the camera information (start)
    prEntry =create_proc_entry("rgk_cameraInfo",044,NULL);
    if (prEntry) {
         prEntry->read_proc  = rgk_read_camera_info;
	 prEntry->write_proc = NULL;
    }
    else {
        PK_ERR("add /proc/cameraInfo entry fail \n");
    }
  //zhoulidong  add it for the camera information (end)
    atomic_set(&g_CamHWOpend, 0); 
