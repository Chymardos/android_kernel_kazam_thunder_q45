...
struct wake_lock cm36283_ps_lock;
 ...
 * Sysfs attributes
*******************************************************************************/
static ssize_t cm36283_show_chipinfo(struct device_driver *ddri, char *buf)
{
	int res;
...
//add BUG_ID:None tianchunming 20130722(start)
static ssize_t ps_canc_read(struct device_driver *ddri, char *buf)
{
	cm36283_enable_ps(cm36283_obj->client, 1);	
	int res;
	u8 databuf[2];
	APS_LOG("WKP ps_canc_read\n");
	if(!cm36283_obj)
	{
		APS_ERR("cm36283_obj is null!!\n");
		return 0;
	...
	databuf[0] = CM36283_REG_PS_CANC;
	CM36283_i2c_master_operate(cm36283_obj->client, databuf, 0x201, I2C_FLAG_READ);
	cm36283_enable_ps(cm36283_obj->client, 0);
	return snprintf(buf, PAGE_SIZE, "0x%04X\n", databuf[0]);
}


static ssize_t cm36283_ps_write(struct device_driver *ddri, const char *buf, size_t count)
{
	//APS_LOG("WKP ps_write:count:%d\n", count);	
	if(!cm36283_obj)
	{
		APS_ERR("cm36283_obj is null!!\n");
		return 0;
	...
	//add a max calibration value
	if(databuf[1] >= 15)
	{
		databuf[1] = 3;
	}
	//APS_LOG("WKP databuf[1] = %d\n",databuf[1]);	
	databuf[2]= 0;	

	long res = CM36283_i2c_master_operate(cm36283_obj->client,databuf, 0x3, I2C_FLAG_WRITE);
	cm36283_enable_ps(cm36283_obj->client, 0);		
	return count;
}

static ssize_t cm36283_ps_read(struct device_driver *ddri, char *buf)
{
	cm36283_enable_ps(cm36283_obj->client, 1);
	int res;
	u8 databuf2[2];
	APS_LOG("WKP cm36283_ps_read\n");
	if(!cm36283_obj)
	{
		APS_ERR("cm36283_obj is null!!\n");
		return 0;
	...
	databuf2[0] = CM36283_REG_PS_CANC;
	CM36283_i2c_master_operate(cm36283_obj->client, databuf2, 0x201, I2C_FLAG_READ);
	cm36283_enable_ps(cm36283_obj->client, 0);
	return snprintf(buf, PAGE_SIZE, "0x%04X\n", databuf2[0]);		
...
/*---------------------------------------------------------------------------------------*/
//add BUG_ID:None tianchunming 20130722(start)
static DRIVER_ATTR(psclb,   S_IWUSR | S_IRUGO, cm36283_ps_read, cm36283_ps_write);
static DRIVER_ATTR(ps_canc, S_IWUSR | S_IRUGO, ps_canc_read, NULL);
//add BUG_ID:None tianchunming 20130722(end)
...
static DRIVER_ATTR(chipinfo,      S_IWUSR | S_IRUGO, cm36283_show_chipinfo, NULL);
...
static int cm36283_setup_eint(struct i2c_client *client)
{
	struct cm36283_priv *obj = i2c_get_clientdata(client);        
		...
		switch (cmd)
		{
			...
			/*----------------------------------for factory mode test---------------------------------------*/
        	  case	ALSPS_GET_PS_TEST_RESULT:
				if((err = cm36283_read_ps(obj->client, &obj->ps)))
				{
				    ps_result = 0;//add BUG_ID:None tianchunming 20130722				
                ...
                {  
                	ps_result = ninterrupt;
                }	
                //modify BUG_ID:None tianchunming 20130722(end)
...
static int cm36283_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	wake_lock_init(&cm36283_ps_lock,WAKE_LOCK_SUSPEND,"cm36283_ps wakelock");//hxh
