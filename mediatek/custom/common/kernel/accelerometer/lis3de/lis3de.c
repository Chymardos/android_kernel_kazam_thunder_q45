...
static struct i2c_board_info __initdata i2c_LIS3DE={ I2C_BOARD_INFO("LIS3DE", (0x52>>1))};
...
/*----------------------------------------------------------------------------*/
int lis3de_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static struct device_driver *platform_gsensor_driver = NULL ;
int lis3de_exist = 0;
#endif
static int lis3de_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#ifdef GSENSOR_BE_COMPATIABLE 
        lis3de_exist = 1;
	 platform_gsensor_driver = driver_find("gsensor",lis3de_gsensor_driver.driver.bus);
  	 if(platform_gsensor_driver ==NULL)
    	{
		GSE_ERR("platform_gsensor_driver not registed yet\n");
		goto exit_misc_device_register_failed;
	}
    if((err = lis3de_create_attr(platform_gsensor_driver)))
	#else
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = lis3de_gsensor_operate;
...
static int lis3de_i2c_remove(struct i2c_client *client)
{
	int err = 0;	
	#ifdef GSENSOR_BE_COMPATIABLE 
	if((err = lis3de_delete_attr(platform_gsensor_driver)))
	#else
