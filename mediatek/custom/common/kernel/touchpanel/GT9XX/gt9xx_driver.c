...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    sensor_id_g = rd_cfg_buf[GTP_ADDR_LENGTH];
	...
	printk("### line = %d, abs_x_max = %d, abs_y_max = %d", __LINE__, abs_x_max,abs_y_max);
...
// for TP info
	if (sensor_id_g  == BAOMING_SENSOR_ID)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","BaoMing",client->addr);
	else if (sensor_id_g == LIHE_SENSOR_ID) 
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","LiHe",client->addr);
	else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","Unkown",client->addr);
