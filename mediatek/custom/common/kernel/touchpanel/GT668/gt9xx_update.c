...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
    ...
    if (file_ready == 0)
    {
    #if 1 //GTP_DEF_FW_UPDATE   //modified by bernard 20130716
			  ...
			  char *buf)
{
	...
	if((i2c_smbus_read_i2c_block_data(i2c_client, 0xa8, 1, &tp_factory_id_cmd))< 0)
        ...
        //get hardware info
        ret = gup_get_ic_msg(i2c_client_point,GUP_REG_HW_INFO, buf_info,4);
        if(FAIL == ret)
        {
          GTP_ERROR("SYS:Read hardware info fail.");
        }
        memcpy(sys_update_msg.ic_fw_msg.hw_info, &buf_info[GTP_ADDR_LENGTH], 4);
        for(i = 0; i < 4; i++)
        {
          sys_update_msg.ic_fw_msg.hw_info[i] = buf_info[GTP_ADDR_LENGTH + 3 -i];
        }
        GTP_INFO("IC Hardware info:%02x%02x%02x%02x",sys_update_msg.ic_fw_msg.hw_info[0],sys_update_msg.ic_fw_msg.hw_info[1],sys_update_msg.ic_fw_msg.hw_info[2],sys_update_msg.ic_fw_msg.hw_info[3]);

        //get firmware info
        for(retry = 0; retry < 2; retry++)
        {
          ret = gup_get_ic_msg(i2c_client_point, GUP_REG_FW_MSG, buf_info, 1);
          if(FAIL == ret)
          {
            GTP_ERROR("Read firmware message fail.");
          }
          sys_update_msg.force_update = buf_info[GTP_ADDR_LENGTH];
        }
        GTP_INFO("IC force update flag:0x%x",sys_update_msg.force_update);
        ...
        //get pid & vid
        ret = gup_get_ic_msg(i2c_client_point, GUP_REG_PID_VID, buf_info, 6);
        if(FAIL == ret)
        {
          GTP_ERROR("Read product id & version id fail.");
        }
        memset(sys_update_msg.ic_fw_msg.pid, 0, sizeof(sys_update_msg.ic_fw_msg.pid));
        memcpy(sys_update_msg.ic_fw_msg.pid, &buf_info[GTP_ADDR_LENGTH], 4);
        if(sys_update_msg.ic_fw_msg.pid[0] != 0)
        {
          if(!memcmp(sys_update_msg.ic_fw_msg.pid,"9111",4))
          {
            GTP_INFO("IC Mapping Product id:%s",sys_update_msg.ic_fw_msg.pid);
            memcpy(sys_update_msg.ic_fw_msg.pid, "9110P", 5);
          }
        }
        sys_update_msg.ic_fw_msg.vid = buf_info[GTP_ADDR_LENGTH + 4] + (buf_info[GTP_ADDR_LENGTH + 5] << 8);
        
        s += sprintf(s, "IC Hardware info:%02x%02x%02x%02x\n",sys_update_msg.ic_fw_msg.hw_info[0],sys_update_msg.ic_fw_msg.hw_info[1],sys_update_msg.ic_fw_msg.hw_info[2],sys_update_msg.ic_fw_msg.hw_info[3]);
        s += sprintf(s, "IC Mapping Product id:%s\n",sys_update_msg.ic_fw_msg.pid);
        s += sprintf(s, "IC FW version id:%02x\n",sys_update_msg.ic_fw_msg.vid);
...
struct kobject *gt9xx_tpswitch_ctrl_kobj;
...
int tpswitch_sysfs_init(void)
{ 
	gt9xx_tpswitch_ctrl_kobj = kobject_create_and_add("tp_compatible", NULL);
	if (!gt9xx_tpswitch_ctrl_kobj)
		return -ENOMEM;

	return sysfs_create_group(gt9xx_tpswitch_ctrl_kobj, &tpswitch_attr_group);
}

//static void tpswitch_sysfs_exit(void)
void tpswitch_sysfs_exit(void)
{
	sysfs_remove_group(gt9xx_tpswitch_ctrl_kobj, &tpswitch_attr_group);

	kobject_put(gt9xx_tpswitch_ctrl_kobj);
}
/*===========================SYS DEBUG END============================*/
