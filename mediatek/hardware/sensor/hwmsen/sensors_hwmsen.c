					...
					for(i =0; i < MAX_ANDROID_SENSOR_NUM; i++)
					{
						...
						if(i == 1){
							...
							if(MSENSOR_ACCURANCY != dev->sensors[i].acceleration.status)
								...
								ALOGD("M_ACC %d, %f",dev->sensors[i].acceleration.status,time);
							...
							mode_temp = (int)(result/100);

							if(mode_value != mode_temp)
								{
								float time = (float)((float)(dev->sensors[i].timestamp)/(float)1000000000);
								ALOGD("M_DA: %f,  %d, %f",result,dev->sensors[i].acceleration.status,time);
