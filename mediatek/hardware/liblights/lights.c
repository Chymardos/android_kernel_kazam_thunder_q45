...
//        	write_int(RED_LED_FILE, level); // default full brightness
		...
		while (((access(RED_DELAY_OFF_FILE, F_OK) == -1) || (access(RED_DELAY_OFF_FILE, R_OK|W_OK) == -1)) && i<10) {
			...
			led_wait_delay(5);//sleep 2ms for wait kernel LED class create led delay_off/delay_on node of fs
			i++;
		}
        ...
        struct light_state_t const* state)
{
	...
	// modify by huangkunming add 7 color leds (start)
	/*if (alpha) {
    ...
    if (red) {
		...
		blink_blue(0xFF0000FF, onMS, offMS);
		blink_green(0xFF00FF00, onMS, offMS);
    }
    else if (green) {
        blink_red(0, 0, 0);
        blink_blue(0, 0, 0);
    ...
    if (red && green && blue) { // white color
        blink_red(0xFFFF0000, onMS, offMS);
		blink_green(0xFF00FF00, onMS, offMS);
		blink_blue(0xFF0000FF, onMS, offMS);
    }
	else if (red && green) { // yellow color
        blink_red(0xFFFF0000, onMS, offMS);
		blink_green(0xFF00FF00, onMS, offMS);
        blink_blue(0, 0, 0);
    }
	else if (red && blue) { // magenta color
        blink_red(0xFFFF0000, onMS, offMS);
        blink_green(0, 0, 0);
        blink_blue(0xFF0000FF, onMS, offMS);
    }
	else if (green && blue) { // indigo color
        blink_red(0, 0, 0);
        blink_green(0xFF00FF00, onMS, offMS);
        blink_blue(0xFF0000FF, onMS, offMS);
    }
	else if (red) { // red color
        blink_red(0xFFFF0000, onMS, offMS);
        blink_green(0, 0, 0);
        blink_blue(0, 0, 0);
    }
	else if (green) { // green color
        blink_red(0, 0, 0);
        blink_green(0xFF00FF00, onMS, offMS);
        blink_blue(0, 0, 0);
    }
	else if (blue) { // blue color
        blink_red(0, 0, 0);
        blink_green(0, 0, 0);
        blink_blue(0xFF0000FF, onMS, offMS);
    }
	...
	// modify by huangkunming add 7 color leds (end)
