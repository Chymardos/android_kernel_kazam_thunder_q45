...
static kal_int32 rgk_bat_vol = 0;  //zhoulidong add 
...
{ 
    ...
    //zhoulidong add(start)
         ...
         if( BMT_status.charger_exist == KAL_TRUE ) 
         {
                ...
                rgk_bat_vol = battery_meter_get_battery_voltage();    
                battery_xlog_printk(BAT_LOG_CRTI,"rgk_bat_vol=%d\n",rgk_bat_vol);
...
//zhoulidong add 
kal_int32 rgk_get_bat_vol(void)
{
        return rgk_bat_vol;
...
void check_battery_exist(void)
{
    ...
    if( baton_count >= 3)
    {
        ...
        else
        {
			...
			battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
          //  battery_charging_control(CHARGING_CMD_SET_PLATFORM_RESET,NULL);   
          //pengjinlong change,device will reboot when plug in usb without battery
        }
    }    
#endif
}
