...
extern unsigned int UbVbat;   //add ALPS01188644 
extern void rgk_switch_pchr_turn_on_charging (void);  //zhoulidong add
...
void dod_init(void)
{
    ...
    #endif
    //D209 DELJ-37
    //zhoulidong add ,afterrecovery flinish, boot reason is RTC.we shoud read RTC
...
void oam_init(void)
{
	...
	kal_uint32 charging_enable = KAL_TRUE; //zhoulidong add
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_SENSE, &vol_bat);
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);

//add ALPS01188644 (start)
         bm_print(BM_LOG_CRTI,"~~RGK  gFG_voltage(%d) \n ",gFG_voltage);
         bm_print(BM_LOG_CRTI,"~~RGK  UbVbat(%d) \n ",UbVbat);
	if(abs(UbVbat-gFG_voltage)>40)
	{ 
...
//zhoulidong add(start)
         ...
         if( BMT_status.charger_exist == KAL_TRUE ) 
         {
                ...
                vol_bat = battery_meter_get_battery_voltage();    
                battery_xlog_printk(BAT_LOG_CRTI,"vol_bat = %d\n",vol_bat);
                rgk_switch_pchr_turn_on_charging();  
         }
#endif
//zhoulidong add(end)
...
extern kal_int32 rgk_get_bat_vol(void);
...
void oam_run(void)
{
 ...
 //zhoulidong add(start)
         ...
         if( BMT_status.charger_exist == KAL_TRUE ) 
         {
                bm_print(BM_LOG_CRTI,"rgk_oam_run:enter vol_bat: %d \n ",vol_bat); 
                    if( vol_bat -rgk_get_bat_vol()>0 )
                                 vol_bat = rgk_get_bat_vol();
                bm_print(BM_LOG_CRTI,"rgk_oam_run:exit   vol_bat: %d \n ",vol_bat); 
         }
#endif
//zhoulidong add(end)
